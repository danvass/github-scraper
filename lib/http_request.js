...
module.exports = function simple_http_request (path, callback,userOptions) {
  ...
  if (userOptions) {
    ...
    if (userOptions.port) options.port = userOptions.port;
    if (userOptions.hostname) options.hostname = userOptions.hostname;
    if (userOptions.pathFn) options.path = userOptions.pathFn(options.path);
  }
